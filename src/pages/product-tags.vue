...
const getItems = () => {
  settingsListStore.fetchProductTags({
    q: searchQuery.value,
  }).then(response => {
  ...
  <div>
    <VCard>
      <VCardTitle class="d-flex align-center">
        ...
        <span class="mx-1">{{ t('Product_Tags') }}</span>
      </VCardTitle>
        ...
        <tbody>
          ...
          >
            ...
            <td class="d-flex align-center gap-2">
              <span :style="{backgroundColor: item.color, display: 'inline-block' , width: '15px', height: '15px'}"></span>
<!--              <span class="mx-1">{{ item.color }}</span>-->
            </td>
            <td>
              <VBtn
                icon
    ...
    <AddProductTagsDialog v-model:isAddOpen="isAddOpen" :products="products" @refreshTable="getItems" />
    <EditProductTagsDialog
      v-model:isEditOpen="isEditOpen"
      :products="products"
      :item="selectedItem"
      @refreshTable="getItems"
    />
    <DeleteProductTagsDialog
