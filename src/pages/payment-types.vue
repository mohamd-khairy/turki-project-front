...
import DeletePaymentTypeDialog from "@core/components/pages/DeleteDialogs/DeletePaymentTypeDialog.vue"
...
const getItems = () => {
  settingsListStore.fetchPaymentTypes({
    q: searchQuery.value,
  }).then(response => {
  ...
  <div>
    <VCard>
      <VCardTitle class="d-flex align-center">
        ...
        <span class="mx-1">{{ t('Payment_Types') }}</span>
      </VCardTitle>
      ...
      <VTable class="text-no-wrap product-list-table">
        ...
        >
          ...
          <td>
            <VIcon icon="ph:dot-bold" :color="item.active === true || item.active === 1 ? '#008000' : '#f00000'"
                   size="32"
            ></VIcon>
            <span>
                {{
                item.active === true || item.active === 1 ? t('forms.statuses.active') : t('forms.statuses.inactive')
              }}
              </span>
          </td>
          <td>
    ...
    <AddPaymentTypeDialog v-model:isAddOpen="isAddOpen" @refreshTable="getItems"/>
